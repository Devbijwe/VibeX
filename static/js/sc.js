...
let searchBtn = document.getElementById("pills-search-tab");
...
let playBadge = document.getElementsByClassName("playBadge");
let songli = document.getElementsByClassName("songli");
...
let songIdArr = []
let datasongId = document.getElementsByClassName("datasongIds")
Array.from(datasongId).forEach((elem) => {
    songIdArr.push(elem.innerHTML)
})
...
function songSelection(index) {
    ...
    try {
        Array.from(songList).forEach((elem, i) => {
            if (i == index) {
                playBadge[i].innerHTML = `<i style="transform: scale(1.6)" class="fa-solid fa-pause"></i>`;
                songli[i].classList.remove("list-group-item-dark");
                songli[i].classList.add("list-group-item-primary");
                // elem.setAttribute("aria-current", "true")
            } else {
                playBadge[i].innerHTML = `<i style="transform: scale(1.6)" class="fa-solid fa-play"></i>`;
                songli[i].classList.remove("list-group-item-primary");
                songli[i].classList.add("list-group-item-dark");
...
// document.getElementById("card").addEventListener("dblclick", playPause);
document.getElementById("card").addEventListener("dblclick", getClickedPosition)
...
function getClickedPosition(e) {
    ...
    if (xPos <= 150 && (yPos < 540 && yPos > 80)) {
    ...
    } else if (xPos >= 1385 && (yPos < 540 && yPos > 80)) {
        audio.currentTime = audio.duration;
        ...
        songSelection(index)
    } else if ((xPos > 150 && xPos < 1385) && (yPos < 540 && yPos > 80)) {
...
let seqAllInterval = setInterval(seqAllSong, 4000);
repeatBtn.addEventListener("click", () => {
    if (repeatBtnCount % 3 == 0) {
        ...
        repeatAllInterval = setInterval(repeatAllSong, 4000);
        ...
        clearInterval(seqAllInterval)
        clearInterval(shuffleInterval)
        repeatOnceInterval = setInterval(repeatSong, 4000);
        shuffleBtn.setAttribute("style", "color:white;")
        ...
        clearInterval(repeatAllInterval);
        clearInterval(shuffleInterval);
        seqAllInterval = setInterval(seqAllSong, 4000);
    }
    repeatBtnCount++;
    shuffleBtnCount = 0;
})
...
shuffleBtn.addEventListener("click", () => {
    if (shuffleBtnCount % 2 == 0) {
        ...
        clearInterval(repeatAllInterval);
        clearInterval(seqAllInterval);
        shuffleInterval = setInterval(shuffleSong, 4000);
    } else {
        shuffleBtn.setAttribute("style", "color:white;");
        clearInterval(shuffleInterval);
    }
